...
### 解耦的附带收益：可以用小模型替换 GPT-3.5 当 Planner

ReWOO 把系统拆成了三个独立角色。既然是独立的，那**能不能用一个便宜的小模型来扮演其中一个角色？** 论文的回答是：可以，而且因为 Planner 的输入输出极其简单，训练小模型来当 Planner 非常容易。

```
先理解"蒸馏"是什么意思——用大模型的输出当老师，教小模型模仿:
───────────────────────────────────────────────────────────────
...
**为什么 Planner 特别容易被替换，而 ReAct 做不到这一点？**
