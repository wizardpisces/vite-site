...
![Transformer 架构全图：Encoder-Decoder 结构，左侧为 N 层 Encoder，右侧为 N 层 Decoder，中间通过 Multi-Head Attention 连接](https://arxiv.org/html/1706.03762v7/Figures/ModalNet-21.png)

### 2. 输入嵌入 (Input Embedding)：从离散符号到连续向量

在进入 Attention 之前，模型需要先把离散的 Token ID 转换为连续的向量表示。这一步由 **Embedding 矩阵** 完成。

*   **本质：** 一个形状为 $V \times d_{model}$（词表大小 × 隐藏维度）的**可学习查找表**。每个 Token ID 对应矩阵中的一行向量。例如词表中 "apple" 的 ID 是 3726，那么 Embedding 操作就是直接取出矩阵的第 3726 行。

*   **怎么训练？** Embedding 矩阵就是模型的一个普通参数，随整个 Transformer 端到端反向传播更新，没有单独的预训练阶段。初始化时通常用随机值，训练过程中语义相近的词（如 "king" 和 "queen"）的向量会自然地靠近。

*   **缩放因子 $\sqrt{d_{model}}$：** 原论文中，Embedding 的输出会乘以 $\sqrt{d_{model}}$。原因是：随机初始化的 Embedding 向量每个维度的值很小（方差约 $1/d_{model}$），而 Positional Encoding 的值域在 $[-1, 1]$。如果不缩放，位置信号会淹没语义信号。乘以 $\sqrt{d_{model}}$ 后，两者的数值量级才能匹配。

*   **完整数据流：**
    $$ \text{Token ID} \xrightarrow{\text{Embedding Lookup}} \text{词向量} \xrightarrow{\times \sqrt{d_{model}}} \text{缩放后的词向量} \xrightarrow{+ PE} \text{模型输入} $$

### 3. 深度解构：串行堆叠的 Layer (Vertical Stacking)
...
![Scaled Dot-Product Attention：Q、K、V 的矩阵运算流程，包含缩放、Softmax 与加权求和](https://arxiv.org/html/1706.03762v7/Figures/ModalNet-19.png)

#### Self-Attention 核心代码实现
...
class SelfAttention(nn.Module):
    def __init__(self, d_model):
        ...
        self.value = nn.Linear(d_model, d_model)  # W_v
    ...
    def forward(self, x, mask=None):
        ...
        # scores[i,j] 表示第i个词对第j个词的关注度(未归一化)
        
        # 步骤3: 缩放 (防止梯度消失)
        scores = scores / math.sqrt(d_model)  # [B, S, S]
        
        # 步骤4: 应用掩码 (可选,用于 Decoder 的因果注意力)
        if mask is not None:
            scores = scores.masked_fill(mask == 0, -1e9)
        
        # 步骤5: Softmax 归一化
        attn_weights = torch.softmax(scores, dim=-1)  # [B, S, S]
        # attn_weights[i,j] 表示第i个词对第j个词的关注权重(已归一化,和为1)
...
# 假设线性投影后的 Q, K, V (简化,实际上会通过 W_q, W_k, W_v 变换)
Q = torch.tensor([[
    [1.2, 0.6, 0.3, 0.9],  # "The" 的查询向量
    [0.4, 1.0, 0.5, 0.2],  # "cat" 的查询向量
    [0.8, 0.3, 0.7, 0.6]   # "sat" 的查询向量
]])

K = torch.tensor([[
    [1.0, 0.4, 0.2, 0.7],  # "The" 的键向量
    [0.5, 0.9, 0.3, 0.1],  # "cat" 的键向量
    [0.6, 0.2, 0.8, 0.4]   # "sat" 的键向量
]])

V = torch.tensor([[
    [2.0, 1.5, 1.0, 2.5],  # "The" 的值向量
    [1.8, 2.2, 1.3, 1.1],  # "cat" 的值向量
    [1.5, 1.8, 2.0, 1.6]   # "sat" 的值向量
]])

# 步骤1: 计算注意力分数 Q·K^T
scores = torch.matmul(Q, K.transpose(-2, -1))
# scores[0, 0, :] = [1.2,0.6,0.3,0.9] · [[1.0,0.4,0.2,0.7],
#                                          [0.5,0.9,0.3,0.1],
#                                          [0.6,0.2,0.8,0.4]]^T
# 结果: scores = [[[2.43, 1.44, 1.68],    # "The" 对 "The", "cat", "sat" 的分数
#                  [1.88, 1.37, 1.24],    # "cat" 对各词的分数
#                  [1.95, 1.21, 1.72]]]   # "sat" 对各词的分数
# shape: [1, 3, 3]

# 步骤2: 缩放
scores = scores / math.sqrt(4)  # 除以 sqrt(d_model)
# scores = [[[1.215, 0.72, 0.84],
#            [0.94, 0.685, 0.62],
#            [0.975, 0.605, 0.86]]]

# 步骤3: Softmax 归一化
attn_weights = torch.softmax(scores, dim=-1)
# attn_weights = [[[0.41, 0.26, 0.33],   # "The" 的注意力: 主要关注自己(41%)
#                  [0.36, 0.31, 0.33],   # "cat" 的注意力: 较均匀分布
#                  [0.35, 0.26, 0.39]]]  # "sat" 的注意力: 主要关注自己(39%)
# 每行和为1,表示注意力权重的分配

# 步骤4: 加权求和 (获取最终的上下文向量)
output = torch.matmul(attn_weights, V)
# 对于 "The" (第1个词):
# output[0, 0, :] = 0.41*[2.0,1.5,1.0,2.5] + 0.26*[1.8,2.2,1.3,1.1] + 0.33*[1.5,1.8,2.0,1.6]
#                 = [1.79, 1.76, 1.40, 1.87]
# 这个向量融合了 "The", "cat", "sat" 三个词的上下文信息
# shape: [1, 3, 4]
```

**关键理解:**
1. **Q·K^T** 计算词与词之间的相似度(未归一化的注意力分数)
2. **Softmax** 将分数转换为概率分布(权重和为1)
3. **乘以V** 根据权重加权提取各词的内容向量,得到融合了上下文的新表示
...
![Multi-Head Attention：多个并行的 Scaled Dot-Product Attention 头，经 Concat 与线性变换后输出](https://arxiv.org/html/1706.03762v7/Figures/ModalNet-20.png)

#### Multi-Head Attention 核心代码实现
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        维度变换流程:
            [B, S, D] -> [B, S, D] -> [B, H, S, d_k] -> [B, H, S, d_k] -> [B, S, D]
            输入 -> 投影 -> 切分头 -> Attention -> 拼接 -> 输出投影
        ...
        # 步骤2: 切分为多头
        Q = self.split_heads(Q)  # [B, H, S, d_k]
        K = self.split_heads(K)  # [B, H, S, d_k]
        V = self.split_heads(V)  # [B, H, S, d_k]
        ...
        # 步骤5: 拼接多头
        attn_output = attn_output.transpose(1, 2).contiguous()  # [B, S, H, d_k]
        attn_output = attn_output.view(batch_size, -1, self.d_model)  # [B, S, D]
        
        # 步骤6: 输出投影
        output = self.W_o(attn_output)  # [B, S, D]
...
假设:
- **B=1**, **S=3**, **D=8** (d_model=8)
- **H=2** (2个头), 因此 **d_k=4** (每个头4维)

```python
x = torch.randn(1, 3, 8)  # 输入: [1, 3, 8]

# 经过 W_q 投影后
Q = W_q(x)  # [1, 3, 8]

# 切分为2个头
Q_split = Q.view(1, 3, 2, 4).transpose(1, 2)  # [1, 2, 3, 4]
# 现在有2个头,每个头处理3个词,每个词4维
# Q_split[0, 0, :, :] 是第1个头看到的所有词
# Q_split[0, 1, :, :] 是第2个头看到的所有词

# 每个头独立计算 Attention
# 头1: Q[0,0,:,:] @ K[0,0,:,:].T @ V[0,0,:,:]
# 头2: Q[0,1,:,:] @ K[0,1,:,:].T @ V[0,1,:,:]

# 最后拼接: [1, 2, 3, 4] -> [1, 3, 2, 4] -> [1, 3, 8]
```

**多头的优势:**
1. **不同视角**: 头1可能关注语法关系,头2关注语义相似性
2. **降低风险**: 即使某个头学坏了,其他头可以补救
3. **并行计算**: 所有头可以同时计算,不增加串行时间
