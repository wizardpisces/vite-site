...
当 Prompt 开始同时描述多个互相制约的目标（如"深入但别猜""全面但别啰嗦"），说明问题已经从 Prompt Engineering 演变成了系统设计问题。

# [Agentic coding and persistent returns to expertise](https://www.anthropic.com/research/claude-code-expertise)

研究发现，用户在具体任务上的专业程度越高，Claude 每次指令后能完成的工作越多，成功率也越高。
文章举的意思是：一个不会 Python 的会计，如果非常清楚对账规则，也可能很好地指挥 Claude 写出有用脚本。

得出一个众所周知的结论：AI 编程工具可能会替代一部分“纯实现型”工作，但不会替代领域判断。未来更有价值的可能是：清楚知道问题是什么、标准是什么、如何判断结果对不对的人。

# [Paving the way for agents in biology](https://www.anthropic.com/research/agents-in-biology)

文章真正想说的是基础设施问题

团队开发了 gget virus，它把 NCBI Virus 复杂的网页检索逻辑封装成可复现、可审计、机器可调用的工具。
加上这个工具后，所有智能体准确率都超过 90%，最高到 99.7%，多次运行的波动也基本消失。模型之间差距也明显缩小。
要让 AI 真正参与科学发现，底层的数据访问、标识符、schema、元数据、坐标系统、过滤逻辑必须更可靠、更标准、更适合 agent 调用。

其他行业也是如此，比如 AI Coding Agent 想要做好，从需求文档开始就得规范化。
...
核心理念：‌
在AI协作中，任何一项任务的算力消耗（Token/推理时间/模型调用），都会自发膨胀，直到耗尽所有已被分配的算力预算——无论这个预算看起来多充裕。

一句话版：‌
你以为给了员工无限的Token，换来的会是效率和深度；实际上，你只是把“摸鱼”变成了“摸算力”。

**洞察二：平庸的“模型升级”**

核心理念：‌
经典定律说“不称职的官员会找两个更平庸的助手”。今天，这变成了“一个简单的需求，会被拆解成多个高配模型参与的多智能体协作流程”。

现象描述：‌

本可以用轻量模型一句话搞定的总结 → 变成了用最强模型先总结、再让另一个模型“挑战”、再让第三个模型“美化”的算力黑洞
最终输出可能和最初版本毫无差异，但消耗的Token翻了几十倍

**洞察三：算力充裕，反而杀死了“用脑子”**

核心理念：‌
帕金森定律真正可怕的，不是时间或算力被消耗——而是“优化”和“精简”的动机，在资源无限供给的环境中被系统性地消灭了。
...
节约不会带来奖励，只会让下次的预算变少。
而挥霍，却能证明你在“充分使用先进生产力”。

这就是帕金森定律在数字时代的冷酷翻版：‌不是算力不够用，而是“用脑子省算力”这件事，在组织行为学上已经死了。‌

* https://mp.weixin.qq.com/s/G0TazN_cHs2crJaNTrt4FA

# [Natural Language Autoencoders](https://www.anthropic.com/research/natural-language-autoencoders)

它想解决一个问题：我们现在不只想看模型“说了什么”，还想知道它在输出前内部大概“想到了什么”。

做法也挺有意思：把模型中间层 activation 翻译成自然语言，再看这段话能不能还原回原 activation。相比之前 SAE / circuit tracing 还要研究员看 feature 和 graph 再解释，NLA 更像是让内部状态直接用人话暴露出来。

这里的输入不是模型参数，也不是最终预测词的 embedding，而是某个 token 位置在某一层的 hidden state / residual stream activation。可以理解成“模型准备预测下一个 token 前，某一层里的上下文状态”。例如 `The capital of France is` 要预测 `Paris`，NLA 读的不是 `Paris` 这个词本身，而是最后一个输入位置在中间层的向量；这个向量里可能已经包含“法国首都”“候选答案 Paris”等信息。

SAE（Sparse Autoencoder，稀疏自编码器）和 circuit tracing 之前也能解释模型内部，但它们更像是把 activation 拆成 feature、graph，再由研究员命名和验证。NLA 的变化是把自然语言放进训练闭环：`activation -> 文字解释 -> 重建 activation`。如果文字能帮助重建原 activation，说明这段解释至少保留了一部分内部状态信息。

但 NLA 也不是绝对更可靠。SAE / circuit tracing 的优势是更可验证，尤其是可以做 ablation、activation patching、feature steering；NLA 的优势是更直接、更快暴露主题，但解释可能 hallucinate，需要再用 SAE、circuit tracing 或行为实验交叉验证。

所以 NLA 没有解决“模型一换、解释工具就要重训/校准”的问题，也没有完整读取整个推理过程。它解决的主要是“内部状态太难读、人工解释太慢”的问题：把某一层、某个 token 的局部状态先翻译成人能快速扫描的线索。

我觉得最值得看的点是：它可能帮助发现模型没说出口的东西，比如“我知道自己正在被评测”。这对后续 agent safety / eval 设计会挺有参考价值。

# Agent Skill 沉淀的理解

从 Agent 运行过程沉淀 Skill 的本质，是从概率性的探索轨迹中，蒸馏出可复用的确定性经验模式，将偶然的成功转化为必然的能力。

# [Gemini 图像分割任务为什么建议关闭 thinking？](https://ai.google.dev/gemini-api/docs/interactions/image-understanding?hl=zh-cn#segmentation)

“thinking”优化的是语言推理路径，不是坐标回归路径；box 圈选依赖的是视觉 grounding，而普通 thinking 会把原本应直接从视觉特征到坐标的任务，改造成‘先语言解释，再由解释推坐标’的间接任务。这个中间层会丢失空间精度。
也就是 box 圈选需要的是低层视觉空间对齐；thinking 强化的是高层语言语义推理。两者目标函数不完全一致，所以 thinking 可能把定位任务从“看图定框”带偏成“按解释猜框”。导致精度变低。

# 模型越强，越不需要 ai code review？

AI 越强，代码审查越不能省——关于 AI Code Review 的四个核心认知‌

1. 局部正确 ≠ 系统安全。‌ 模型能写出语法完美的代码片段，但真实工程的脆弱点往往藏在跨模块交互、边界遗漏和回归风险中。AI Code Review 的核心价值，是把审查视角从“这段代码对不对”升级为“这段代码放进系统里会不会出事”。

2. 同模型审查依然有效，甚至更关键。‌ 编码和审查是两种认知任务。当审查 Agent 拥有独立的上下文窗口、不共享生成阶段的推理链路时，它更容易发现生成者因注意力偏差而忽略的问题——本质上是通过上下文独立性来降低同源偏差。

3. 代码产量正在指数级膨胀，人工审查已经崩了。‌ AI 编程工具让单月代码产量从 2.5 万行飙升至 25 万行，传统逐行精审模式彻底失效。没有 AI Code Review 作为第一道质量闸门，大量代码将带着未经验证的风险直接进入主干。

4. 范式正在转移：从“事后审查”到“事前治理”。‌ 未来的质量控制将被前移到代码生成之前——人类定义规格与护栏，AI 负责生成，独立 AI 审查 Agent 进行对抗性验证。人类不再逐行看代码，而是聚焦于“我们是否在解决正确的问题”。

结论：‌ 模型能力越强，越需要一个独立的、专门用于验证的 AI 审查环节。生成和验证，从来都应该是分离的。

* [生成不是瓶颈，验证才是：为什么 AI 越强越需要 Code Review](https://mp.weixin.qq.com/s/8V-qVvb5ScESxe_bUMuqvQ)

# AI 应用的风险分层自治

一个很重要的产品洞察：成熟 AI 应用往往不是简单地“全自动”或“全人工审核”，而是采用 **低风险自动接受，中高风险进入审核** 的分层自治策略。

核心原因是 AI 产品同时面对两个目标：

* **效率**：低风险、可逆、影响范围小的任务应该尽量自动完成，否则 AI 只会变成另一个需要人操作的界面。
* **可靠性**：中高风险、不可逆、影响钱/健康/法律/生产系统/组织决策的任务，必须保留人类判断、授权和追责。

更准确的判断公式：

```text
风险 ≈ 出错概率 × 影响范围 × 不可逆性 × 权限级别 × 合规/声誉成本
```

因此，风险分层不应该只看模型置信度，而要看动作本身的后果。

## 在知识服务里的体现

知识服务领域尤其适合这种策略，因为知识服务的风险不只是“操作失败”，而是错误知识会影响人的判断。

* **低风险知识**：自动整理、自动回答、自动沉淀。例如办公地址、基础流程、普通 FAQ、公开产品说明。
* **中风险知识**：AI 生成结构化结论，但需要引用来源、标注适用范围，并进入人工快速审核。例如客服 SOP、技术排障、价格政策、内部流程边界。
* **高风险知识**：AI 只做检索、摘要、证据整理和风险提示，不直接替人下最终结论。例如法律、财税、医疗、合规、安全、生产事故处理。

知识服务的关键不是让 AI “记住更多内容”，而是让 AI 在正确权限、正确时间、正确风险等级下，找到可信证据并生成可追溯答案。

## 对知识库生成的启发

知识库不应该只是“把文档切块丢进向量库”，而应该生成一套可追溯、可审核、可更新的知识资产。

合理的知识库可以分层：

```text
原始资料层：制度、合同、文档、网页、工单、会议纪要
...
* **低风险**：AI 自动抽取、自动质检、自动发布。
* **中风险**：AI 抽取与结构化，人类审核后发布。
* **高风险**：专家审核、版本管理、强制引用来源。
* **极高风险**：不进入自动回答，只作为人工辅助材料。

这个原则本质上是 **risk-based autonomy**：让 AI 在低风险区域释放速度，在高风险区域保留人类判断权。

---

# 可读性训练

## 什么是 Legibility Training？
字面意思"可读性训练"。做法是：在训练 AAR 时，除了奖励 PGR，还额外奖励"方法可解释"。比如：
...
Anthropic 的立场是："宁可牺牲一些性能，也要保持可解释性"，但在竞争压力下这种立场能不能坚持得住，是个悬而未决的问题。

# [Karpathy 的 LLM Wiki：预编译知识库模式](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)

Karpathy 于 2026 年 4 月提出 LLM Wiki 模式：不再让 LLM 在每次查询时从原始文档碎片中重新检索和综合（传统 RAG），而是让 LLM **在摄入（ingest）阶段就增量构建和维护一个持久化的 wiki**——结构化的、互相链接的 Markdown 文件集合。知识被编译一次、持续维护，而非每次查询都重新推导。

**三层架构**
* **Raw Sources**：不可变的原始文档（论文、文章、数据），LLM 只读不写
* **Wiki**：LLM 生成维护的 Markdown 文件（摘要、实体页、概念页、对比、综述），LLM 拥有完全写入权
* **Schema**：CLAUDE.md / AGENTS.md 风格的规约文件，定义 wiki 结构规范和工作流，人与 LLM 共同演化

**三个核心操作**
* **Ingest**：摄入新源文档 → LLM 阅读理解 → 写摘要页 → 更新实体页/概念页 → 标注矛盾 → 更新 index.md（单次 ingest 可能触及 10-15 个 wiki 页面）
* **Query**：读 index.md 定位相关页面 → 读取已编译的 wiki 内容 → 综合回答（跨文档关联在 ingest 时已建立，无需每次重新发现）
* **Lint**：定期健康检查——找矛盾、过时信息、孤立页面、缺失概念页、缺失交叉引用

**核心启发**
* **"编译"vs"检索"的思维转换**：传统 RAG 是惰性求值（query time retrieve + synthesize），LLM Wiki 是预计算（ingest time 完成 synthesis），类似物化视图 vs 每次 join 查询的区别。对知识密集、需跨文档综合的场景效率更高。
* **查询结果反哺知识库**：好的问答结果可以沉淀回 wiki 成为新页面，形成知识复利的正循环。
* **Lint = 知识库的代码质量检查**：把软件工程的 lint 理念引入知识管理，定期发现矛盾、过时、孤立等问题。
* **Schema 即 prompt engineering 的制度化**：将 LLM 从"通用聊天机器人"约束为"有纪律的 wiki 维护员"。

**"杀死 RAG"？本质是 RAG 的进化形态**

LLM Wiki 并没有消除 Retrieval——wiki 规模增长后，query 阶段仍需通过 index.md 或搜索引擎（如 qmd）定位相关页面。本质区别是**检索对象从原始文档 chunks 升级为 LLM 编译后的知识制品**，和微软 GraphRAG（先提取实体构建知识图谱再检索）思路高度一致。更准确的描述是：**预编译知识层 + 对编译产物的 RAG**。

**方案的核心问题**
* **错误固化与传播**：传统 RAG 检索的是原始文档（即使检索不完美，chunk 本身是真实的）。LLM Wiki 检索的是 LLM 编译产物——如果编译阶段理解错误、遗漏、过度总结，错误会固化在 wiki 中，后续每次查询都在错误基础上回答。更危险的是错误会传播放大：第 N 次 ingest 基于之前的错误摘要做出错误的交叉引用，信息溯源链条断裂。传统 RAG 的"无状态"反而是安全特性——每次从源头重新来，不积累 LLM 的理解偏差。
* **成本随规模递增**：传统 RAG 的 ingest 成本几乎恒定（切 chunk + embedding）。LLM Wiki 的每次 ingest 需要 LLM 读新文档 + 读 index + 读 10-15 个已有页面 + 逐个改写，可能消耗数万到十几万 token，且成本随 wiki 规模增长而递增。
* **一致性维护是指数级难题**：200 个 wiki 页面时，任何新事实理论上可能影响其中任意多个页面。LLM 无法保证找到所有需要更新的页面，本质上是分布式系统的一致性问题——只不过"分布式节点"换成了 Markdown 文件。Lint 操作本身也受限于 context window 和 LLM 注意力。
* **不可审计、不可回溯**：传统 RAG 可以看到"答案基于哪几个原始 chunk"，可追溯验证。LLM Wiki 的答案基于被多轮修改过的 wiki 页面，原始信息经过多轮"传话"，除非对 wiki 做 git 版本管理并仔细 diff，否则无法回溯。
* **场景局限**：最适合知识型、结构化、变化慢、容错率高的个人研究场景。对话记录/会议纪要（信息密度低）、代码仓库（变化太快）、法律/合规文档（不能丢细节）、实时数据（来不及编译）等场景难以适用。企业级应用需要的安全性、审计性、可扩展性均缺失。
* **Schema 冷启动与演化痛苦**：初始结构定义困难，定错后前期 ingest 产物可能全需重构，和代码大规模重构一样痛苦且不可完全信任 LLM 的重构结果。

**更现实的方向**：混合模式——用 LLM Wiki 做高层知识地图和导航索引，query 时仍回溯原始文档做最终验证。把"编译"当做缓存层而非唯一信源。

---

# [Measuring AI Ability to Complete Long Software Tasks](https://emptysqua.re/blog/review-measuring-ai-ability-to-complete-long-software-tasks/)

METR 定义了"时间视界"（Time Horizon）指标：AI agent 以给定成功率完成的任务，若由人类专家来做需要多长时间。核心发现：50% 成功率的时间视界每 7 个月翻一倍（GPT-2: 2秒 → Claude 3.7 Sonnet: 50分钟 → o3: ~2小时 → Opus 4.6: ~12小时）。按趋势线推算，2027-2031 年间前沿 AI 将能无监督完成人类专家需一个月的任务。

核心启发

* **Benchmark ≠ 真实能力**：SWE-bench 中约一半通过自动测试的 PR 会被人类维护者拒绝（违反约定、破坏无关代码、未真正修复 bug）。"60% SWE-bench" 不等于 "60% 可接受"——自动测试永远无法覆盖所有隐性规则。
* **"Messiness"（混乱度）是 AI 的真正瓶颈**：论文定义了 16 个降低 AI 表现的因素——不可逆错误、有限资源消耗、因果不可区分、成功难以度量等。混乱度对 AI 的阻碍大于人类，但 AI 在混乱任务上的进步速度与整洁任务相当。
* **Test Oracle 决定上限**：如果任务有明确的测试判定器（test oracle），AI 在 5 年内大概率能以 50% 成功率完成需人类一个月的任务。但真实软件工作中正确性模糊、无裁判、错误不可逆——整洁 benchmark 与混乱现实之间的鸿沟巨大。
* **软件架构的终极问题**：如果 AI 时间视界持续增长，微服务、模块化、关注点分离等架构原则——部分是为人类有限认知服务的——是否还有必要？我们可能正从"手工锻造软件"（精心打磨、经久耐用、修改昂贵）进入"注塑软件"时代（廉价到可以随时丢弃重做）。
* **AGI 的时间视界是"无穷大"**：论文指出 AGI 意味着无限时间视界，因此长期趋势将快于指数增长，在 AGI 部署日出现渐近线。

---

# AI Agent 协同探索解空间

| 方法         | 多路径策略                | 协调机制     | 环境/反思反馈      | 成本可控性          |
| ---------- | -------------------- | -------- | ------------ | -------------- |
| **CoT-SC** | 多条独立推理链              | 无协调      | 无            | 中等（生成路径数量可控）   |
| **ToT**    | 树状多路径（固定每节点扩展 c 个分支） | 无动态协调    | 无            | 可控（固定分支数）      |
| **LATS**   | 动态多路径（MCTS 搜索）       | 闭环反馈选择路径 | 有（环境反馈 + 反思） | 中等偏低（搜索成本高）    |
| **FoA**    | 粒子/代理多路径探索           | 粒子滤波式协调  | 无反思机制        | 高（粒子数量可调，成本可控） |

CoT-SC → ToT → LATS：演进核心是 搜索策略复杂化 + 引入反馈/反思

LATS → FoA：演进核心是 从反思驱动切换到高效协调与成本控制

FoA 与 LATS 最大区别：FoA 优化资源与协作效率，但不主动反思；LATS 注重路径优化和反思，但计算成本较高。
...
# AI Agent 学习资源（按学习路线 & 抽象深度排序）

| 阶段 | 名称 | 类型 | 学习价值 / 核心思想 | 工程复杂度 | 抽象深度 | 链接 |
|---|---|---|---|---|---|---|
| **入门实验** | smolagents | 框架/代码 | 轻量 agent，CodeAgent + ToolCallingAgent，快速理解动作循环 | 低 | 低 | [github.com/huggingface/smolagents](https://github.com/huggingface/smolagents) |
| **入门实验** | BabyAGI | 框架/代码 | 极简任务循环，任务生成 + 执行 + 回写 memory | 低 | 中 | [github.com/yoheinakajima/babyagi](https://github.com/yoheinakajima/babyagi) |
| **原型思想** | ReAct | 论文+代码 | Thought + Action + Observation 循环，agent 核心范式 | 低 | 高 | 论文：[arxiv.org/abs/2210.03629](https://arxiv.org/abs/2210.03629)，代码：[github.com/ysymyth/ReAct](https://github.com/ysymyth/ReAct) |
| **原型思想** | Pre-Act | 论文+代码 | 多步规划 + 细化 reasoning，在 ReAct 基础上优化 agent | 中 | 高 | 论文：[arxiv.org/abs/2505.09970](https://arxiv.org/abs/2505.09970)，代码：[github.com/Fu-Dayuan/PreAct](https://github.com/Fu-Dayuan/PreAct) |
| **搜索推理** | Tree of Thoughts | 论文+代码 | 多分支推理 + branch evaluation，复杂 reasoning agent | 中 | 高 | 论文：[arxiv.org/abs/2305.10601](https://arxiv.org/abs/2305.10601)，代码：[github.com/princeton-nlp/tree-of-thought-llm](https://github.com/princeton-nlp/tree-of-thought-llm) |
| **长期 memory / skill** | Generative Agents | 论文 | Memory-driven agent，episodic memory + reflection + long-term behavior | 中 | 高 | [arxiv.org/abs/2304.03442](https://arxiv.org/abs/2304.03442) |
| **长期 memory / skill** | Voyager | 论文 | Curriculum + skill accumulation，持续进化 agent | 高 | 高 | [arxiv.org/abs/2304.09644](https://arxiv.org/abs/2305.16291) |
| **长期 memory / skill** | Reflexion | 论文+代码 | Agent 从失败中学习，verbal self-reflection + trial-and-error | 低 | 高 | 论文：[arxiv.org/abs/2303.11366](https://arxiv.org/abs/2303.11366)，代码：[github.com/noahshinn/reflexion](https://github.com/noahshinn/reflexion) |
| **核心控制流** | LangGraph | 框架/代码 | 状态机控制流，LangChain 核心 agent orchestration | 中 | 高 | [github.com/langchain-ai/langgraph](https://github.com/langchain-ai/langgraph) |
| **核心控制流** | OpenAI Agents SDK | 框架/代码 | 极简 agent 框架设计（Agent / Tool / Handoff / Guardrail） | 低 | 中高 | [github.com/openai/openai-agents-python](https://github.com/openai/openai-agents-python) |
| **核心控制流实战** | DeepAgents | 框架/代码 | LangGraph 实战案例，planning + sub-agent + context（灵感来自 Claude Code） | 中 | 高 | [github.com/langchain-ai/deepagents](https://github.com/langchain-ai/deepagents) |
| **多 agent 协作** | AutoGen | 框架/代码 | 多 agent dialogue + actor model，理解协作架构（已进入维护模式，见 [microsoft/agent-framework](https://github.com/microsoft/agent-framework)） | 中 | 中高 | [github.com/microsoft/autogen](https://github.com/microsoft/autogen) |
| **多 agent 协作** | CrewAI | 框架/代码 | 事件/角色驱动 agent，LangChain-independent Flows | 中 | 中 | [github.com/crewaiinc/crewai](https://github.com/crewaiinc/crewai) |
| **多 agent 协作研究** | Fleet of Agents | 论文+代码 | 分布式 agent 协作 + 粒子滤波式决策（ICML 2025） | 高 | 高 | 论文：[arxiv.org/abs/2405.06691](https://arxiv.org/abs/2405.06691)，代码：[github.com/au-clan/FoA](https://github.com/au-clan/FoA) |
| **约束生成（辅助）** | Guidance | 框架/代码 | token 级约束生成，控制 LLM 输出结构（JSON/正则/CFG），微软出品 | 低 | 中 | [github.com/guidance-ai/guidance](https://github.com/guidance-ai/guidance) |
| **平台化 / 产品级** | Auto-GPT | 框架/代码 | 自主 agent 平台，早期循环架构可参考学习 | 高 | 中 | [github.com/Significant-Gravitas/AutoGPT](https://github.com/Significant-Gravitas/AutoGPT) |
| **平台化 / 产品级** | OpenClaw | 框架/代码 | 本地个人助手 + multi-agent routing，学习原理价值低 | 高 | 中低 | [github.com/openclaw/openclaw](https://github.com/openclaw/openclaw) |

推荐学习路径：
* **学原理**：smolagents → LangGraph → DeepAgents → AutoGen
* **学生产级架构**：LangGraph → OpenAI Agents SDK → DeepAgents → CrewAI Flows
* **快速出 demo**：DeepAgents → CrewAI
...
* 表层原因（数据层面）：
    * SFT：使用的是非同策略（Off-policy）数据（即外来的、非模型生成的数据）。
    * RL：使用的是同策略（On-policy）数据（模型自己生成并探索的数据）。
* 深层原因（权重层面 - 核心差异）：
    * SFT（破坏根基）：权重矩阵的主分量（Principal Components）会被外来数据大幅修改。
    * 后果：导致模型“根基”不稳，引发灾难性遗忘（Catastrophic Forgetting）和过拟合。
    * RL（修剪枝叶）：权重矩阵的主分量保持不变，改变的只是次要分量。
    * 后果：权重的变化分布更稀疏（Sparse），能在保留原有能力的基础上优化行为，避免了灾难性遗忘。
...
# [Steam, Steel, and Infinite Minds - Notion CEO Ivan Zhao 的 AI 革命思考](https://www.notion.com/blog/steam-steel-and-infinite-minds-ai)
...
## "按需加载"的实现原理：两阶段 Prompt 注入（非 Embedding 检索）

**阶段一：描述注入（始终在 system prompt 中）**
* Cursor 在每次对话时，将所有已注册 skill 的 description（最长 1024 字符）直接注入 system prompt 的 ```
<available_skills>
