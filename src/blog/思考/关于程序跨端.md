...
* WebAssembly 很多语言都可以编译成 .wasm，被 WebAssembly instantiate 后在浏览器或者node环境运行
    ...
    * 优势
        * 体积小：由于浏览器运行时只加载编译成的字节码，一样的逻辑比用字符串描述的 JS 文件体积要小很多；(按道理JS源码会更小；其他语言编译的 .wasm 会大很多，信息里面需要包含除了逻辑代码外的信息)
