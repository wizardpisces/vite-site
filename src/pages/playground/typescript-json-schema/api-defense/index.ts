...
export function validateAndFixAPI<T>(res: Response<T>, schema: Record<string, any>): Response<T> {
    ...
    try { // 避免校验过程中出现错误，而导致业务代码无法运行
        ...
        if (!valid) {
            ...
            validate.errors?.forEach(error => { // 实际错误长度一般只有第一个遇到的错误，也就是数组长度是 1，所以会从头到尾扫描 schema 补全，也简化了操作
                if (error.keyword === 'type') {
                     ...
                     */
                    reportError({ other: error, data: res, msg: 'type misMatch', schema })
                    ...
                    */
                    reportError({ other: error, data: res, schema, msg: 'missing property' })
                } else {
                    reportError({ other: error, data: res, schema, msg: 'unknown error' })
                }
        ...
        reportError({ msg: 'validateAndFixAPI exception', schema, data: res, other: e })
    }
